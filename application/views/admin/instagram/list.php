<div id="content">
    <h2>Instagram &raquo; Pending List</h2>
    <h2>Total <?php echo digits($total_item);?></h2>
    <div id="submenu">
        <ul>  
            <li>  
           	<a class="defBtn" href="<?php echo site_url('lifebuoy_adm/instagram/save_db/').'/'.($hashtag);?>"><span>Get Latest</span></a>
            </li>  	
        </ul>
    </div>
     <form action="<?php echo site_url('lifebuoy_adm/instagram/search_pending');?>" method="post">
    ...
    <table class="defAdminTable" width="100%">
		...
		$no=$no+($this->per_page*($offset/($this->per_page)));
            ...
            <tr>
                ...
                <input type="checkbox" name="id[]" value="<?php echo $list['id'];?>" class="selection"><?php /*?><br><a href="<?php echo site_url('lifebuoy_adm/instagram/do_reject').'/'.$list['id'];?>" onClick="return confirm('Reject?');">Reject</a><?php */?>
                </td>
	...
	$(document).ready(function(){
		$(".submit_btn").click(function(){
			...
			if(action_type!=2){
				if(action_type==1){
					if(confirm("Approve?")){
						$("#twit_pending_form").attr('action','<?php echo site_url('lifebuoy_adm/instagram/batch_approve');?>');
					...
					if(confirm("Reject?")){
						$("#twit_pending_form").attr('action','<?php echo site_url('lifebuoy_adm/instagram/batch_reject');?>');
						$("#twit_pending_form").removeAttr('onsubmit');
						$("#twit_pending_form").submit();
					}
				}
		...
		$(".update_btn").click(function(){
			if(confirm("Update Data?")){
				$("#twit_pending_form").attr('action','<?php echo site_url('lifebuoy_adm/instagram/batch_update');?>');
				$("#twit_pending_form").removeAttr('onsubmit');
				$("#twit_pending_form").submit();
			}
		});	

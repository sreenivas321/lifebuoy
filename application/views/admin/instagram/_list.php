<div id="content">
    ...
    <div id="submenu">
        <ul>  
            <li>  
			<?php if($prev_link!=1){?>
            <a class="defBtn" href="<?php echo site_url('lifebuoy_adm/instagram/'.$hashtag.'/'.$prev_link.'/01');?>"><span>PREV</span></a>
            <?php }?>
			<?php if($pagination!=11){?><a class="defBtn" href="<?php echo site_url('lifebuoy_adm/instagram/'.$hashtag.'/01').'/'.$pagination;?>"><span>NEXT</span></a><?php }?>       
            </li>  	
        </ul>
    </div>
           ...
           <?php $no=1; 
            ...
            <tr>
          	...
          	<?php if(!in_array($list['src'],$post_list)){?>
                  ...
                  <td>           
                <select name="category_id" id="category_<?php echo $no;?>" class="txtField" style="width:100px;">
                <option value="0">== Select Category ==</option>
                <?php if($category)foreach($category as $list2){?>
                <option value="<?php echo $list2['id'];?>"   <?php if(isset($category_list[$list['src']]))echo "selected=\"selected\"";?>><?php echo $list2['name'].'('.$list2['hashtag'].')';?></option>
                <?php }?>
                </select>
                <input type="hidden" name="post_date" id="post_date_<?php echo $no?>" value="<?php echo date("Y-m-d H:i:s",$list['post_date'])?>" />
				</td>
                <td valign="top"><?php echo $list['fullname'].'('.$list['username'].')';?></td>
                <td valign="top"><a href="<?php echo $list['url'];?>"><img src="<?php echo $list['thumb']?>" /></a></td>
      			 <td valign="top" style="word-wrap: break-word !important; word-break:normal;"><?php echo wordwrap($list['caption'],50,"<br>",true);?></td>
                 <td valign="top"><?php echo date("j F Y H:i",$list['post_date'])?></td>
    		</tr>
			<?php $no++; ?>	
			 <?php }else if(!$images_list){ ?> 
            <tr>
                <td colspan="7" align="center"><?php echo "No Instagram List"; ?></td>
        	</tr>
            <?php } ?>
        </tbody>
    </table>
...
function approve_data(id){
	if(confirm('Approve?')){
			...
			var post_date=$("#post_date_"+id).val();
			//alert (username);
			$.ajax({
			type: "POST",
			url: "<?php echo site_url('lifebuoy_adm/instagram/do_approve');?>",
			data: {
						image_src : image_src,
						caption:caption,
						username:username,
...
function reject_data(id){
	if(confirm('Rejected?')){
			...
			$.ajax({
			type: "POST",
			url: "<?php echo site_url('lifebuoy_adm/instagram/do_reject');?>",
			data: {
						image_src : image_src,
						caption:caption,
						username:username,

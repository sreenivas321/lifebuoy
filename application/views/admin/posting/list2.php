<script>
function do_filter(){
window.location='<?php echo site_url('lifebuoy_adm/posting/inactive');?>/'+$("#category_id").val()+'/'+$("#sorting").val();
}
</script>
<div id="content">
    <h2>Approved &raquo; Inactive List</h2>
    <h2>Total <?php echo digits($total_item);?></h2>
     <form action="<?php echo site_url('lifebuoy_adm/posting/inactive');?>" method="get">
    ...
    <option value="2" <?php if($sorting==2)echo "selected=\"selected\"";?>>Most Dislike</option>
        ...
        <dl>
            <dd></dd>
            <dt><input type="submit" class="defBtn search_btn" value="Filter" /></dt>
        </dl>
    </form>	
    ...
    <table class="defAdminTable" width="100%">
		...
		$no=$no+($this->per_page*($this->input->get('page')/($this->per_page)));
            ...
            <tr>
                ...
                <td valign="top"><?php  if($list['post_via']==0)echo 'Twitter';else if($list['post_via']==1)echo "Instagram"; else if($list['post_via']==2)echo "Web Upload";else echo "Facebook";?></td>
                <td valign="top">
				...
				<?php echo $list['fullname'].'('.$list['username'].')';?></td>
            ...
            <tr>
                <td colspan="8" align="center"><?php echo "No Approved List"; ?></td>
        	</tr>
            <?php } ?>
        </tbody>
    </table>

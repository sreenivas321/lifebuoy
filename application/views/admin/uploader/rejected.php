...
<select name="change_filter" onchange="change_filter(this.value);" id="change_filter" class="txtField" style="width:300px">
    ...
    <h2>Uploader Rejected &raquo; List</h2>
    <h2>Total <?php echo digits($total_item);?></h2>
  
    <form id="twit_pending_form" action="<?php echo site_url('lifebuoy_adm/uploader/update_data_rejected');?>" method="post">
...
function change_filter(view){
	offset=0;
	category_id='a';
		window.location='<?php echo site_url('lifebuoy_adm/uploader/rejected');?>/'+category_id+'/'+view+'/'+offset+'/';	
}
</script>